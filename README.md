...
- وابستگی‌های Python را از فایل (`⁦requirements.txt⁩`) نصب می‌کند.
- برنامه اصلی را اجرا می‌کند.

در اجراهای بعدی، برنامه مستقیماً باز می‌شود.
...
## روش دوم — استفاده از ارائه‌دهنده آنلاین (`⁦Online Provider⁩`) با کلید API — پیشنهادی

این روش برای ارزیابی سریع‌تر و ساده‌تر پروژه **پیشنهاد می‌شود**.

برای تست بخش ارائه‌دهنده‌های آنلاین، یک کلید آزمایشی (`⁦API Key⁩`) به‌صورت خصوصی در اختیار استاد قرار داده می‌شود.

> کلید آزمایشی عمداً داخل مخزن عمومی (`⁦GitHub Repository⁩`) قرار داده نشده است. استاد فقط کافی است کلید ارسال‌شده را در فیلد مربوط به کلید API داخل برنامه وارد کند.

### مراحل اجرا

**مرحله ۱ — دانلود پروژه**

مخزن پروژه را دانلود کنید و فایل ZIP را از حالت فشرده خارج کنید.

```text
Repository → Download ZIP → Extract
```

**مرحله ۲ — اجرای برنامه**

فایل راه‌انداز برنامه را اجرا کنید:

```text
START.cmd
```

**مرحله ۳ — ورود به بخش ارائه‌دهنده‌های API**
...
**مرحله ۴ — انتخاب Provider**

ارائه‌دهنده مربوط به کلید آزمایشی ارسال‌شده را انتخاب کنید.

برای مثال، اگر کلید مربوط به Groq باشد:

```text
Groq
```

**مرحله ۵ — وارد کردن API Key**

کلید آزمایشی ارسال‌شده را در فیلد زیر وارد کنید:

```text
API Key
```

**مرحله ۶ — دریافت فهرست مدل‌ها**
...
**مرحله ۷ — انتخاب Model**

یکی از مدل‌های نمایش‌داده‌شده را انتخاب کنید.
...
برای بررسی صحت اتصال، گزینه زیر را انتخاب کنید:

```text
Test
```

**مرحله ۹ — فعال‌سازی Provider**

در صورت موفق بودن تست و در صورت نیاز، ارائه‌دهنده را فعال کنید:
...
وارد بخش گفت‌وگو شوید:

```text
Chat
```

یک پیام آزمایشی ارسال کنید. اگر پاسخ دریافت شود، ارتباط برنامه با ارائه‌دهنده آنلاین با موفقیت برقرار شده است.
...
این روش برای بررسی قابلیت اجرای مدل به‌صورت محلی است و به کلید API خارجی نیاز ندارد.

پس از اجرای فایل راه‌انداز:
...
پس از نصب Runtime، گزینه زیر را انتخاب کنید:

```text
Download Lite model
```

پس از دانلود مدل، گزینه زیر را انتخاب کنید:

```text
Start model
```

صبر کنید وضعیت مدل به حالت آماده برسد:

```text
Ready
```

سپس وارد بخش گفت‌وگو شوید:

```text
Chat
```

و یک پیام آزمایشی ارسال کنید.
...
Runtime (`⁦Runtime⁩`) و Model (`⁦Model⁩`) داخل Repository قرار داده نشده‌اند تا مخزن GitHub سبک باقی بماند.
...
اطلاعات زیر نباید داخل مخزن عمومی (`⁦Repository⁩`) Commit شوند:
...
همچنین فایل‌های Runtime، Model و محیط مجازی (`⁦Virtual Environment⁩`) توسط `⁦.gitignore⁩` از Repository خارج نگه داشته شده‌اند.
